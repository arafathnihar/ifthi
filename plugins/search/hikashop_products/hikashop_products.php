		...
		if($count){
			...
			foreach ( $rows as $k => $row ) {
				...
				if($variants && $row->product_type=='variant'){
					...
					$row->product_name=$mains[$row->product_parent_id]->product_name;
				}
				$row->text=$row->product_description;
				if(method_exists($app,'stringURLSafe')){
					$alias = $app->stringURLSafe(strip_tags($row->product_name));
				}else{
					$alias = JFilterOutput::stringURLSafe(strip_tags($row->product_name));
				}
				$rows[$k]->href = 'index.php?option=com_hikashop&ctrl=product&task=show&name='.$alias.'&cid='.$row->id.$Itemid.$parent;
