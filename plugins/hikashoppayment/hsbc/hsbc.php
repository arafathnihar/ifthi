...
?><?php
class plgHikashoppaymentHSBC extends JPlugin
{
	...
	function onPaymentDisplay(&$order,&$methods,&$usable_methods) {
		if(!empty($methods)){
			foreach($methods as $method){
				if($method->payment_type!='hsbc' || !$method->enabled){
					continue;
				}
	...
	function onAfterOrderConfirm(&$order,&$methods,$method_id) {
		...
		$notify_url = $httpsHikashop.'index.php?option=com_hikashop&ctrl=checkout&task=notify&notif_payment=hsbc&tmpl=component&lang='.$locale;
		$return_url = $httpsHikashop.'index.php?option=com_hikashop&ctrl=checkout&task=notify&notif_payment=hsbc&tmpl=component&hsbc_return=1&lang='.$locale;
		$debug = $method->payment_params->debug?'T':'P';
		$vars = Array(
			...
			"TransactionType" => $method->payment_params->instant_capture?'Capture':'Auth',
			"PurchaseAmount" => round($order->cart->full_total->prices[0]->price_value_with_tax * 100),
			"PurchaseCurrency" => $this->accepted_currencies[ $currency->currency_code ],
			"StorefrontId" => $method->payment_params->merchantid,
			"TimeStamp" => time() . '000',
			"UserId" => $user->user_id,
			"ShopperEmail" => substr($user->user_email, 0, 30)
		);
		...
		$vars["OrderHash"] = $this->generate($vars, $method->payment_params->cpihash);
	...
	function onPaymentNotification(&$statuses){
		...
		$elements = $pluginsClass->getMethods('payment','hsbc');
		...
		$error_url = $httpsHikashop.'index.php?option=com_hikashop&ctrl=order'.$url_itemid;
		...
		if( $_POST['StorefrontId'] != $element->payment_params->merchantid ) {
			if($finalReturn) {
		...
		if(empty($dbOrder)){
			...
			if($finalReturn) {
				$app->enqueueMessage(JText::sprintf('NOTIFICATION_REFUSED_FOR_THE_ORDER','HSBC'),'Invalid order');
				$app->redirect($error_url);
			}
			return false;
		}
		...
		$order->history->history_amount = $vars['PurchaseAmount'].$vars['PurchaseCurrency'];
		...
		if( $orderPrice != $vars['PurchaseAmount'] || $orderCurrency != $vars['PurchaseCurrency'] ) {
			if($finalReturn) {
				$app->enqueueMessage(JText::sprintf('NOTIFICATION_REFUSED_FOR_THE_ORDER','HSBC').JText::_('INVALID_AMOUNT'));
			...
			$orderClass->save($order);
	 		...
	 		$mailer->setSubject(JText::sprintf('NOTIFICATION_REFUSED_FOR_THE_ORDER','HSBC').JText::_('INVALID_AMOUNT'));
			$body = str_replace('<br/>',"\r\n",JText::sprintf('AMOUNT_RECEIVED_DIFFERENT_FROM_ORDER','HSBC',$order->history->history_amount,$price_check.$currency->currency_code))."\r\n\r\n".$order_text;
			$mailer->setBody($body);
			$mailer->Send();
		...
		if($completed) {
			...
			$order_text = $i . ' - ' . $this->errorCpiResultText[$i]."\r\n\r\n".$order_text;

			if($finalReturn) {
				$app->enqueueMessage(JText::sprintf('NOTIFICATION_REFUSED_FOR_THE_ORDER','HSBC').' '.$this->errorCpiResultText[$i]);
				$app->redirect($cancel_url);
			}
		}
	 	$order->mail_status = $statuses[$order->order_status];
	 	$mailer->setSubject(JText::sprintf('PAYMENT_NOTIFICATION_FOR_ORDER','HSBC',$payment_status,$dbOrder->order_number));
		$body = str_replace('<br/>',"\r\n",JText::sprintf('PAYMENT_NOTIFICATION_STATUS','HSBC',$payment_status)).' '.JText::sprintf('ORDER_STATUS_CHANGED',$order->mail_status)."\r\n\r\n".$order_text;
		$mailer->setBody($body);
		$mailer->Send();
		$orderClass->save($order);
	...
	function onPaymentConfiguration(&$element){
		$this->hsbc = JRequest::getCmd('name','hsbc');
		if(empty($element)){
			$element = new stdClass();
			$element->payment_name='HSBC';
			$element->payment_description='You can pay by credit card using this payment method';
			$element->payment_images='MasterCard,VISA,Credit_card,American_Express';
			$element->payment_type=$this->hsbc;
		...
		$this->toolbar = array(
			...
			'cancel',
			'|',
			array('name' => 'pophelp', 'target' =>'payment-hsbc-form')
		);

		hikashop_setTitle('HSBC','plugin','plugins&plugin_type=payment&task=edit&name='.$this->hsbc);
		$app = JFactory::getApplication();
		$app->setUserState( HIKASHOP_COMPONENT.'.payment_plugin_type', $this->hsbc);
