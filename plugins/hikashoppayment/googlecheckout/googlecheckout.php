...
class plgHikashoppaymentGOOGLECHECKOUT extends JPlugin
{
	...
	function onAfterOrderConfirm(&$order,&$methods,$method_id) {
		...
		if(isset($order->order_discount_code)) $data .= '<item><item-name>Order #'.$order->order_id.' - '.JText::_('HIKASHOP_COUPON').'</item-name><item-description></item-description><unit-price currency="'.$currency->currency_code.'">-'.$order->order_discount_price.'</unit-price><quantity>1</quantity></item>';

		if($order->order_shipping_price > 0 ) $data .= '<item><item-name>Order #'.$order->order_id.' - '.JText::_('HIKASHOP_SHIPPING').' </item-name><item-description></item-description><unit-price currency="'.$currency->currency_code.'">'.$order->order_shipping_price.'</unit-price><quantity>1</quantity></item>';
		if($order->order_payment_price > 0 ) $data .= '<item><item-name>Order #'.$order->order_id.' - '.JText::_('HIKASHOP_PAYMENT').' </item-name><item-description></item-description><unit-price currency="'.$currency->currency_code.'">'.$order->order_payment_price.'</unit-price><quantity>1</quantity></item>';
		$tax = $order->cart->full_total->prices[0]->price_value_with_tax - $order->cart->full_total->prices[0]->price_value;
		if($tax>0) $data .= '<item><item-name>Order #'.$order->order_id.' - '.JText::_('TAXES').' </item-name><item-description></item-description><unit-price currency="'.$currency->currency_code.'">'.$tax.'</unit-price><quantity>1</quantity></item>';
	...
	function onPaymentNotification(&$statuses){
		...
		if($vars['state'] == 'CHARGED') {
	 	...
	 	$mailer->setSubject(JText::sprintf('PAYMENT_NOTIFICATION_FOR_ORDER','GOOGLECHECKOUT',$payment_status,$dbOrder->order_number));
	...
	function onPaymentConfiguration(&$element){
		...
		$this->toolbar = array(
			...
			'cancel',
			'|',
			array('name' => 'pophelp', 'target' =>'payment-googlecheckout-form')
		);
