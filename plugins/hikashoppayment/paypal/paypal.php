	...
	function onAfterOrderConfirm(&$order,&$methods,$method_id){
		...
		if(empty($method->payment_params->details)){
			...
			if(!empty($order->order_shipping_price) && bccomp($order->order_shipping_price,0,5)){
				...
				$vars["amount_".$i]=round($order->order_shipping_price-@$order->order_shipping_tax,(int)$currency->currency_locale['int_frac_digits']);
			...
			if(!empty($order->order_payment_price) && bccomp($order->order_payment_price,0,5)){
				...
				$vars["amount_".$i]=round($order->order_payment_price,(int)$currency->currency_locale['int_frac_digits']);
				$vars["quantity_".$i]=1;
				$i++;
			}
			if(bccomp($tax,0,5)){
		...
		if((isset($method->payment_params->validation) && $method->payment_params->validation) || (isset($method->payment_params->enable_validation) && !$method->payment_params->enable_validation)){
			$vars['paymentaction']='authorization';
		}
		if(empty($method->payment_params->url))$method->payment_params->url='https://www.paypal.com/cgi-bin/webscr';
	...
	function onPaymentConfiguration(&$element){
		...
		if($subtask=='ips'){
			...
			$this->toolbar = array(
				...
				'cancel',
				'|',
				array('name' => 'pophelp', 'target' =>'payment-paypal-form')
			);
