...
<tr>
	...
	<td>
		<?php echo JHTML::_('hikaselect.booleanlist', "data[payment][payment_params][details]" , '',@$this->element->payment_params->details	); ?>
	</td>
</tr>
<tr>
	<td class="key">
	...
	<td>
		<?php
		if(!isset($this->element->payment_params->validation)){
			if(isset($this->element->payment_params->enable_validation)) $this->element->payment_params->validation = !$this->element->payment_params->enable_validation;
		}
...
<tr>
	...
	<td>
		...
		<a href="#" onclick="
				try{
					new Ajax('<?php echo hikashop_completeLink('plugins&plugin_type=payment&task=edit&name='.$this->data['paypal'].'&subtask=ips',true,true);?>',
					{
						method: 'get', onComplete: function(result)
						{
							document.getElementById('paypal_ips').value=result;
						}
					}
					).request();
				}catch(err){
					new Request(
					{
						url:'<?php echo hikashop_completeLink('plugins&plugin_type=payment&task=edit&name='.$this->data['paypal'].'&subtask=ips',true,true);?>',
						method: 'get',
