...
class plgHikashoppaymentPaypalpro extends JPlugin
{
	...
	function onBeforeOrderCreate(&$order, &$do) {
		...
		if(empty($order->order_payment_method) || $order->order_payment_method != 'paypalpro') {
			return true;
		}
		...
		$amount = round($order->cart->full_total->prices[0]->price_value_with_tax,(int)$currency->currency_locale['int_frac_digits']);
		...
		if(bccomp($tax,0,5)){
			$vars['TAXAMT']=round($tax+$order->order_shipping_tax-$order->order_discount_tax,(int)$currency->currency_locale['int_frac_digits']);
		}
		if(!empty($order->cart->coupon)){
			$vars["L_NAME".$i]=JText::_('HIKASHOP_COUPON');
			$vars["L_NUMBER".$i]='coupon';
			$vars["L_AMT".$i]=-1*round($order->order_discount_price,(int)$currency->currency_locale['int_frac_digits']);
			$vars["L_QTY".$i]=1;
			$vars["L_TAXAMT".$i]=0;
			$i++;
		}
		...
		if(!empty($order->order_payment_price) && bccomp($order->order_payment_price,0,5)){
			...
			$vars["L_AMT".$i]=round($order->order_payment_price,(int)$currency->currency_locale['int_frac_digits']);
			$vars["L_QTY".$i]=1;
			$vars["L_TAXAMT".$i]=0;
			$i++;
		}
		...
		if( !$error ) {
			...
			if( isset($responseCode) ) {
				...
				if( $responseCode == 'SUCCESS' || $responseCode == 'SUCCESSWITHWARNING' ) {
					...
					$order->history->history_notified = (int)@$method->payment_params->send_notification;
			...
			$app->enqueueMessage('An error occurred. The connection to the PayPal Pro server could not be established: '.curl_error($session));
			$do = false;
		}
		curl_close($session);
	...
	function onPaymentConfiguration(&$element){
		...
		$this->toolbar = array(
				...
				'cancel',
				'|',
				array('name' => 'pophelp', 'target' =>'payment-paypalpro-form')
			);
