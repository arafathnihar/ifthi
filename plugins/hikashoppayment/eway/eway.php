...
class plgHikashoppaymentEway extends JPlugin
{
	...
	function onBeforeOrderCreate(&$order,&$do){
		...
		if(empty($order->order_payment_method) || $order->order_payment_method!='eway'){
			return true;
		}
		...
		switch($eway->doPayment()) {
			...
			case EWAY_TRANSACTION_OK:
				...
				$order->history->history_amount= round($order->cart->full_total->prices[0]->price_value_with_tax,2) . $currency->currency_code;
	...
	function onPaymentConfiguration(&$element){
		...
		$this->toolbar = array(
			...
			'cancel',
			'|',
			array('name' => 'pophelp', 'target' =>'payment-eway-form')
		);
