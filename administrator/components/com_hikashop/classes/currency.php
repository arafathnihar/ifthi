...
class hikashopCurrencyClass extends hikashopClass{
	...
	function getTaxedPrice($price,$zone_id,$tax_category_id,$round=10){
		$this->taxRates=array();
		...
		if(!empty($this->taxRates)){
			foreach($this->taxRates as $k => $rate){
				$this->taxRates[$k]->tax_amount = ($price*floatval($rate->tax_rate));
			}
		}
		return $taxedPrice;
	}
	...
	function getUntaxedPrice($price,$zone_id,$tax_category_id,$round=2){
		...
		if(empty($tax)) return $price;
		$price=(float)$price;
		$untaxedPrice = $price/(1.00000+$tax);
		if(!empty($this->taxRates)){
			foreach($this->taxRates as $k => $rate){
				$this->taxRates[$k]->tax_amount = ($price*floatval($rate->tax_rate)/(1.00000+floatval($rate->tax_rate)));
			}
		}
		return $untaxedPrice;
	}
	...
	function quantityPrice(&$price,$quantity){
		if($quantity>0){
			...
			if(isset($price->price_value_without_discount)){
				$price->unit_price->price_value_without_discount=($price->price_value_without_discount);
				$price->price_value_without_discount=($price->unit_price->price_value_without_discount*$quantity);
			}
			if(isset($price->price_value)){
				$price->unit_price->price_value=($price->price_value);
				$price->price_value=($price->unit_price->price_value*$quantity);
			}
			if(isset($price->price_orig_value)){
				$price->unit_price->price_orig_value=($price->price_orig_value);
				$price->price_orig_value=($price->unit_price->price_orig_value*$quantity);
			}
			if(isset($price->price_orig_value_with_tax)){
				$price->unit_price->price_orig_value_with_tax=($price->price_orig_value_with_tax);
				$price->price_orig_value_with_tax=($price->unit_price->price_orig_value_with_tax*$quantity);
			}
			if(isset($price->price_orig_value_without_discount)){
				$price->unit_price->price_orig_value_without_discount=($price->price_orig_value_without_discount);
				$price->price_orig_value_without_discount=($price->unit_price->price_orig_value_without_discount*$quantity);
			}
			if(isset($price->price_value_without_discount_with_tax)){
				$price->unit_price->price_value_without_discount_with_tax=($price->price_value_without_discount_with_tax);
				$price->price_value_without_discount_with_tax=($price->unit_price->price_value_without_discount_with_tax*$quantity);
			}
			if(isset($price->price_value_with_tax)){
				$price->unit_price->price_value_with_tax=($price->price_value_with_tax);
				$price->price_value_with_tax=($price->unit_price->price_value_with_tax*$quantity);
			}
			...
			if(isset($price->taxes)){
				...
				foreach($price->taxes as $k => $tax){
					...
					$price->taxes[$k]->tax_amount = (@$tax->tax_amount * $quantity);
				}
			}
			if(isset($price->taxes_without_discount)){
				...
				foreach($price->taxes_without_discount as $k => $tax){
					...
					$price->taxes_without_discount[$k]->tax_amount = (@$tax->tax_amount * $quantity);
				}
			}
		}
	}
	...
	function addDiscount(&$price,&$discount,$discount_before_tax,$zone_id,$product_tax_id){
		...
		if($discount_before_tax){
			if(bccomp($discount->discount_flat_amount,0,5)!==0){
				...
				$round = 5;	//$this->getRounding(@$price->price_currency_id);
	...
	function addCoupon(&$prices,&$discount,$products=null,$id=array()){
		...
		if(isset($discount->discount_value)){
			...
			$this->convertCoupon($discount,hikashop_getCurrency());
			$discountClass->recalculateDiscountValue($discount,$products,$id);
			...
			if(bccomp(@$shipping->shipping_price_with_tax,0,5)!==0){
				...
				if($shipping->shipping_price_with_tax!=$shipping->shipping_price){
					if(!isset($shipping->taxes)){
						if(isset($shipping->total->prices[$k]->taxes) && is_array($shipping->total->prices[$k]->taxes)){
							$shipping->taxes = array();
							$tax = reset($shipping->total->prices[$k]->taxes);
							if(is_object($tax)){
					...
					if(isset($shipping->taxes)){
						foreach($shipping->taxes as $tax){
							if(isset($shipping->total->prices[$k]->taxes[$tax->tax_namekey])){
								$shipping->total->prices[$k]->taxes[$tax->tax_namekey]->tax_amount+=$tax->tax_amount;
							}else{
								$shipping->total->prices[$k]->taxes[$tax->tax_namekey] = clone($tax);
							}
						}
					}
				}
