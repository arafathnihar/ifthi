...
class hikashopCartClass extends hikashopClass{
	...
	function update($product_id,$quantity=1,$add=0,$type='product',$resetCartWhenUpdate=true,$force=false){
		...
		if(in_array($type,array('product','item'))){
			...
			if($this->save($cart)){
				...
				$dispatcher->trigger( 'onAfterCartUpdate',array( &$this, &$cart, $product_id, $quantity, $add, $type,$resetCartWhenUpdate,$force,true ));
				return true;
			}
		}

		$dispatcher->trigger( 'onAfterCartUpdate',array( &$this, &$cart, $product_id, $quantity, $add, $type,$resetCartWhenUpdate,$force,false ));
		return false;
	}
	...
	function updateEntry($quantity,&$cartContent,$product_id,$add,$resetCartWhenUpdate=true,$type='product',$force=false){
		...
		}else if ($this->cart_type == 'wishlist' && JRequest::getInt('delete','0') != 1 && !empty($cartContent[$id]->cart_product_id)){
			$query = 'UPDATE '.hikashop_table('cart_product').' SET cart_product_quantity='.(int)$quantity.' WHERE cart_product_id='.(int)$cartContent[$id]->cart_product_id;
			$this->database->setQuery($query);
			$this->database->query();
			$add = 1;
	...
	function _checkQuantity(&$product,&$quantity,&$cartContent,$cart_product_id_for_product){
		...
		if(!empty($this->options[$product->product_id])){
			...
			if(isset($this->new_quantities[$parent]) && $quantity>$this->new_quantities[$parent]){
		...
		if($product->product_min_per_order > 1 && $product->product_min_per_order > $quantity){
	...
	function &_getGlobalQuantityOfVariants(&$cartContent,&$product){
		static $quantity_for_same_main_product=array();
		if(!isset($quantity_for_same_main_product[$product->product_parent_id])){
			$quantity=0;
			if(!empty($cartContent)){
				foreach($cartContent as $element){
					if($element->product_parent_id==$product->product_parent_id) $quantity+=(int)$element->cart_product_quantity;
				}
			}
	...
	function &loadFullCart($additionalInfos=false,$keepEmptyCart=false){
