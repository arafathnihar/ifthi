...
<?php
				...
				echo JText::_('FROM').' ';
				echo JHTML::_('calendar', hikashop_getDate((@$this->pageInfo->filter->filter_start?@$this->pageInfo->filter->filter_start:''),'%Y-%m-%d %H:%M'), 'filter_start','period_start','%Y-%m-%d','size="10" onchange="document.adminForm.submit();"');
				echo ' '.JText::_('TO').' ';
				echo JHTML::_('calendar', hikashop_getDate((@$this->pageInfo->filter->filter_end?@$this->pageInfo->filter->filter_end:''),'%Y-%m-%d %H:%M'), 'filter_end','period_end','%Y-%m-%d','size="10" onchange="document.adminForm.submit();"');
					...
					<td class="hikashop_order_status_value">
						...
						if($this->manage && !$this->popup){
							echo $this->popupHelper->display(
								...
								'ORDER_STATUS',
								'/',
								'status_change_link_'.$row->order_id,
								760, 480, 'style="display:none;"', '', 'link'
							);
							$doc = JFactory::getDocument();
							$doc->addScriptDeclaration(' var '."default_filter_status_".$row->order_id.'=\''.$row->order_status.'\'; ');
							echo $this->category->display("filter_status_".$row->order_id,$row->order_status,'onchange="if(this.value==default_filter_status_'.$row->order_id.'){return;} hikashop.openBox(\'status_change_link_'.$row->order_id.'\',\''.hikashop_completeLink('order&task=changestatus&order_id='.$row->order_id,true).'&status=\'+this.value);this.value=default_filter_status_'.$row->order_id.';if(typeof(jQuery)!=\'undefined\'){jQuery(this).trigger(\'liszt:updated\');}"');
						} else {
