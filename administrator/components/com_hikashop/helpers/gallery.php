...
class hikashopGalleryHelper {
	...
	function __construct() {
		...
		$this->root = JPATH_ROOT . DS . $uploadFolderConfig;
		$externFolder = str_replace(DS, '/', $uploadFolderConfig);
		if($app->isAdmin())
			$this->urlRoot = '../' . $externFolder;
		else
			$this->urlRoot = rtrim(JURI::base(true), '/') . '/' . $externFolder;
	}
