			...
			if(!$ret) {
				switch(strtolower($toolname)) {
					...
					case 'popup':
						...
						if(isset($tool['width'])) {
							if(isset($tool['height'])) {
								$bar->appendButton('HikaPopup', $tool['icon'], $tool['alt'], $tool['url'], $tool['width'], $tool['height']);
							} else {
								$bar->appendButton('HikaPopup', $tool['icon'], $tool['alt'], $tool['url'], $tool['width']);
							}
						} else {
							$bar->appendButton('HikaPopup', $tool['icon'], $tool['alt'], $tool['url']);
						}
						break;
					case 'close':
