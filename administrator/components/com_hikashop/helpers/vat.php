...
class hikashopVatHelper{
	function isValid(&$vat){
		...
		if(empty($zone->zone_code_2) || !in_array($zone->zone_code_2,array('AT','BE','BG','CY','CZ','DK','EE','DE','PT','GR','ES','FI','HU','LU','MT','SI',
		...
		switch($vat_check){
			...
			case 2:
				...
				if($zone->zone_code_2 != strtoupper(substr(str_replace(array(' ','.','-'),array('','',''),$vat_number),0, 2))){
					$app = JFactory::getApplication();
					$this->message = 'Your VAT number can not be used with the country you selected. Please select the right country ( CODE: '.strtoupper(substr(str_replace(array(' ','.','-'),array('','',''),$vat_number),0, 2)).') or make sure your VAT starts with '.$zone->zone_code_2;
					if($_REQUEST['tmpl']=='component'){
						hikashop_display($this->message,'error');
					}else{
						$app->enqueueMessage($this->message);
