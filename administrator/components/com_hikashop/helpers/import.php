...
class hikashopImportHelper{
	...
	function importFromVM(){
		...
		if( isset($_GET['import']) && $_GET['import'] == '1' ) {
			...
			if( $processed ) {
				...
				if( !$this->refreshPage ) {
					echo '<p><a href="'.hikashop_completeLink('import&task=import&importfrom=vm&'.$this->token.'=1&import=1').'">'.JText::_('HIKA_NEXT').'</a></p>';
				}
			} else {
				echo '<a href="'.hikashop_completeLink('import&task=show').'">'.JText::_('HIKA_BACK').'</a>';
			}
		} else {
	...
	function loadConfiguration() {
		...
		$this->vmCatImgDir = HIKASHOP_ROOT . 'components/com_virtuemart/shop_image/categories/';
		...
		foreach($data as $c) {
			if( empty($c->vm_id) ) {
				...
				if( !empty($c->category_full_image)) {
					...
					$this->copyFile($this->vmCatImgDir,$c->category_full_image, $this->options->uploadfolder.$file_name);
				}
				$sep = ',';
	...
	function importProducts() {
		...
		$sql2 = 'INSERT IGNORE INTO `#__hikashop_vm_prod` (`'.implode('`,`',array_keys($data)).'`) '.
			'SELECT '.implode(',',$data).' FROM `#__vm_product` AS vmp INNER JOIN `#__hikashop_product` AS hkp ON CONVERT(vmp.product_sku USING utf8) = CONVERT(hkp.product_code USING utf8) '.
	...
	function importDownloads() {
		...
		$sql = 'INSERT IGNORE INTO `#__hikashop_download` (`'.implode('`,`',array_keys($data)).'`) '.
			...
			'INNER JOIN `#__hikashop_file` AS hkf ON (CONVERT(hkf.file_name USING utf8) = CONVERT(vmd.file_name USING utf8) AND hkp.hk_id = hkf.file_ref_id) '.
	...
	function getStartPage() {
		return 'First, make a backup of your database.<br/>'.
			'When ready, click on <a href="'.hikashop_completeLink('import&task=import&importfrom=vm&'.$this->token.'=1&import=1').'">'.JText::_('HIKA_NEXT').'</a>, otherwise '.
			'<a href="'.hikashop_completeLink('import&task=show').'">'.JText::_('HIKA_BACK').'</a>.';
	}
	...
	function proposeReImport() {
		if( !isset($_GET['reimport']) ) {
			...
			echo '<p><a href="'.hikashop_completeLink('import&task=import&importfrom=vm&'.$this->token.'=1&import=1&reimport=1').'">Import new elements</a></p>';
			return false;
		}
