...
class plgHikashoppaymentSAGEPAY extends JPlugin
{
	...
	function onAfterOrderConfirm(&$order,&$methods,$method_id) {
		...
		if(!function_exists('mcrypt_encrypt')){
			...
			$app->enqueueMessage('The SagePay payment plugin requires the PHP extension Mcrypt to be installed and activated on your server. Please contact your hosting company to set it up');
			return false;
		}
	...
	function onPaymentNotification(&$statuses){
		...
		if( !$completed ) {
			...
			$body = str_replace('<br/>',"\r\n",JText::sprintf('PAYMENT_NOTIFICATION_STATUS','SagePay',$vars['Status'])).' '.JText::_('STATUS_NOT_CHANGED')."\r\n\r\n".$order_text;
		 	$mailer->setSubject(JText::sprintf('PAYMENT_NOTIFICATION_FOR_ORDER','SagePay',$vars['Status'],$dbOrder->order_number));
			$mailer->setBody($body);
			$mailer->Send();
		...
		$mailer->setSubject(JText::sprintf('PAYMENT_NOTIFICATION_FOR_ORDER','SagePay',$vars['payment_status'],$dbOrder->order_number));
	...
	function onPaymentConfiguration(&$element){
		...
		$this->toolbar = array(
				...
				'cancel',
				'|',
				array('name' => 'pophelp', 'target' =>'payment-sagepay-form')
			);
