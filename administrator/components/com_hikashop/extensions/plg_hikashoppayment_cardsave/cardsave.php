...
class plgHikashoppaymentCardSave extends JPlugin
{
	...
	function onBeforeOrderCreate(&$order,&$do) {
		...
		if(empty($order->order_payment_method) || $order->order_payment_method != 'cardsave') {
			return true;
		}
		$db = JFactory::getDBO();
		...
		if(!empty($order->cart->$address_type->address_street)) {
			...
			if(strlen($order->cart->$address_type->address_street)>100) {
				...
				if(empty($address2)) $address2 = @substr($order->cart->$address_type->address_street,100,50);
				if(empty($address3)) $address3 = @substr($order->cart->$address_type->address_street,150,50);
				if(empty($address4)) $address4 = @substr($order->cart->$address_type->address_street,200,50);
			}else{
	...
	function onPaymentNotification(&$statuses){
		...
		if( !$completed ) {
			...
			$body = str_replace('<br/>',"\r\n",JText::sprintf('PAYMENT_NOTIFICATION_STATUS','CardSave',$vars['StatusCode'])).' '.JText::_('STATUS_NOT_CHANGED')."\r\n\r\n".$order_text;
		 	$mailer->setSubject(JText::sprintf('PAYMENT_NOTIFICATION_FOR_ORDER','CardSave',$vars['StatusCode'],$dbOrder->order_number));
			$mailer->setBody($body);
			$mailer->Send();
		...
		$mailer->setSubject(JText::sprintf('PAYMENT_NOTIFICATION_FOR_ORDER','CardSave',$vars['payment_status'],$dbOrder->order_number));
	...
	function onPaymentConfiguration(&$element){
		...
		$this->toolbar = array(
			...
			'cancel',
			'|',
			array('name' => 'pophelp', 'target' =>'payment-cardsave-form')
		);
