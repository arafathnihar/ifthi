...
class plgHikashopshippingCANADAPOST extends JPlugin {
	var $canadapost_methods = array(
	array('name' => 'Priority Courier', 'countries' => 'CANADA'), array('name' => 'Xpresspost', 'countries' => 'CANADA'), array('name' => 'Regular', 'countries' => 'CANADA'),
	array('name' => 'Priority Worldwide USA', 'countries' => 'USA'), array('name' => 'Xpresspost USA', 'countries' => 'USA'), array('name' => 'Small Packets Air', 'countries' => 'USA'), array('name' => 'Small Packets Surface', 'countries' => 'USA'),
	array('name' => 'Priority Worldwide INTL', 'countries' => 'ALL'), array('name' => 'XPressPost International', 'countries' => 'ALL'), array('name' => 'Small Packets Air', 'countries' => 'ALL'), array('name' => 'Parcel Surface', 'countries' => 'ALL'), array('name' => 'Small Packets Surface', 'countries' => 'ALL'), array('name' => 'Expedited', 'countries' => 'ALL'),
	);
	...
	function shippingMethods(&$main) {
		...
		if (!empty($main -> shipping_params -> methods)) {
			foreach ($main->shipping_params->methods as $key => $value) {
				...
				if ($selected) {
					$methods[$selected['name']] = $selected['name'];
				}
			}
		}
		return $methods;
	...
	function onShippingDisplay(&$order, &$dbrates, &$usable_rates, &$messages) {
		if (empty($dbrates)) {
			...
			foreach ($dbrates as $k => $rate) {
				if ($rate -> shipping_type == 'canadapost')
					$rates[] = $rate;
			}
			if (empty($rates)) {
				$messages['no_rates'] = JText::_('NO_SHIPPING_METHOD_FOUND');
			...
			if (bccomp($order -> total -> prices[0] -> price_value, 0, 5)) {
				...
				if (empty($rate -> shipping_params -> shipping_max_price)) {
					...
					if ($price > $rate -> shipping_params -> shipping_max_price) {
						$messages['order_total_too_high'] = JText::_('ORDER_TOTAL_TOO_HIGH_FOR_SHIPPING_METHODS');
						return true;
					}
				}
				...
				foreach ($receivedMethods as $method) {
					...
					if ($method['deliveryDate'] != 'www.canadapost.ca') {
						if (is_numeric($method['deliveryDate'])) {
							...
							$rates[$i] -> shipping_description .= 'Estimated delivery date:  ' . $time;
						} else {
					...
					if ($rate -> shipping_params -> group_package == 1 && $this -> nbpackage > 0)
	...
	function onShippingConfiguration(&$elements) {
		...
		if (empty($elements)) {
			...
			$element -> shipping_type = $this -> canadapost;
			$element -> shipping_params = new stdClass();
			$element -> shipping_params -> post_code = '';
			$element -> shipping_currency_id = $this -> main_currency;
			$elements = array($element);
		}
		$this -> toolbar = array('save', 'apply', array('name' => 'link', 'icon' => 'cancel', 'alt' => JText::_('HIKA_CANCEL'), 'url' => hikashop_completeLink('plugins&plugin_type=shipping')), '|', array('name' => 'pophelp', 'target' => 'shipping-' . $this -> canadapost . '-form'));

		hikashop_setTitle(JText::_('HIKASHOP_SHIPPING_METHOD'), 'plugin', 'plugins&plugin_type=shipping&task=edit&name=' . $this -> canadapost);
		$config = &hikashop_config();
		$this -> main_currency = $config -> get('main_currency', 1);
		$currency = hikashop_get('class.currency');
