...
class plgHikashopshippingUPS extends JPlugin
{
		...
		if(empty($dbrates)){
			...
			if(bccomp($order->total->prices[0]->price_value,0,5)){
				...
				foreach($receivedMethods as $method){
					...
					if($rate->shipping_params->group_package && $this->nbpackage>0) $rates[$i]->shipping_description.='<br/>'.JText::sprintf('X_PACKAGES', $this->nbpackage);
					$i++;
				}
	...
	function onShippingConfiguration(&$elements){
		...
		$this->toolbar = array(
			...
			'|',
			array('name' => 'pophelp', 'target' =>'shipping-'.$this->ups.'-form')
		);
