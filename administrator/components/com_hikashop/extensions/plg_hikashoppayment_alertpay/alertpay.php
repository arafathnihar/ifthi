...
?><?php
class plgHikashoppaymentAlertpay extends JPlugin
{
	var $accepted_currencies = array(
		...
		$method->payment_params->url='https://secure.payza.com/checkout';
		...
		$mailer->addRecipient(explode(',',$config->get('payment_notification_email')));
			...
			$mailer->setSubject(JText::sprintf('NOTIFICATION_REFUSED_FOR_THE_ORDER','Payza').'invalid response');
			$body = JText::sprintf("Hello,\r\n An Payza notification was refused because the notification from the AlertPay server was invalid")."\r\n\r\n".$order_text;
			$mailer->setBody($body);
			$mailer->Send();
			if($element->payment_params->debug){
		...
		if($vars['ap_status']!='Success'){
			$body = str_replace('<br/>',"\r\n",JText::sprintf('PAYMENT_NOTIFICATION_STATUS','Payza',$vars['ap_status'])).' '.JText::_('STATUS_NOT_CHANGED')."\r\n\r\n".$order_text;
		 	$mailer->setSubject(JText::sprintf('PAYMENT_NOTIFICATION_FOR_ORDER','Payza',$vars['ap_status'],$dbOrder->order_number));
			$mailer->setBody($body);
			$mailer->Send();
			if($element->payment_params->debug){
		...
		$currency=$currencies[$dbOrder->order_currency_id];
	 	...
	 	if($price_check != @$vars['ap_totalamount'] || $currency->currency_code != @$vars['ap_currency']){
	 		...
	 		$mailer->setSubject(JText::sprintf('NOTIFICATION_REFUSED_FOR_THE_ORDER','Payza').JText::_('INVALID_AMOUNT'));
			$body = str_replace('<br/>',"\r\n",JText::sprintf('AMOUNT_RECEIVED_DIFFERENT_FROM_ORDER','Payza',$order->history->history_amount,$price_check.$currency->currency_code))."\r\n\r\n".$order_text;
			$mailer->setBody($body);
			$mailer->Send();
	 		return false;
	 	}
	 	...
	 	$mailer->setSubject(JText::sprintf('PAYMENT_NOTIFICATION_FOR_ORDER','Payza',$vars['ap_status'],$dbOrder->order_number));
		$body = str_replace('<br/>',"\r\n",JText::sprintf('PAYMENT_NOTIFICATION_STATUS','Payza',$vars['ap_status'])).' '.JText::sprintf('ORDER_STATUS_CHANGED',$order->mail_status)."\r\n\r\n".$order_text;
		$mailer->setBody($body);
		$mailer->Send();
	 	$orderClass->save($order);
		...
		$this->toolbar = array(
			...
			'cancel',
			'|',
			array('name' => 'pophelp', 'target' =>'payment-alertpay-form')
		);

		hikashop_setTitle('Payza','plugin','plugins&plugin_type=payment&task=edit&name='.$this->alertpay);
		$app = JFactory::getApplication();
