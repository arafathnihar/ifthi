...
?><?php
class plgHikashoppaymentAtos extends JPlugin
{
	...
	var $messageFr=array(
		...
		if(!empty($methods)){
			foreach($methods as $method){
				if($method->payment_type!='atos' || !$method->enabled){
					continue;
				}
				...
				if(!empty($currency) && !empty($currency[$currency_id]->currency_code) && !isset($this->accepted_currencies[$currency[$currency_id]->currency_code])){
					return true;
				}
		...
		$vars = Array(
		"currency_code" => @$this->accepted_currencies[$currency->currency_code],
		...
		if(version_compare(JVERSION,'1.6.0','>=')){
			$db = &JFactory::getDBO();
			$query='SELECT * FROM #__extensions WHERE name=\'plg_system_languagefilter\' AND folder=\'system\' AND enabled=1';
			$db->setQuery($query);
		...
		$vars["customer_email"]=$user->user_email;
		$vars["merchant_id"]=$method->payment_params->merchant_id;
		$vars["merchant_country"]=$method->payment_params->merchant_country;
		$vars["payment_means"]=$method->payment_params->payment_means;

		if($method->payment_params->delay<1 || $method->payment_params->delay>99)
			$vars["delay"]=null;
		else
			$vars["delay"]=$method->payment_params->delay;
		if($method->payment_params->enable_validation==1)
			$vars["capture_mode"]="VALIDATION";
		else
			$vars["capture_mode"]="AUTHOR_CAPTURE";
		if($method->payment_params->instalments>0 && $method->payment_params->instalments<4 && ($method->payment_params->force_instalments==1 || @$_SESSION['hikashop_multiple_instalments']==1)){
			$vars["capture_mode"]="PAYMENT_N";
			$vars["data"]="NB_PAYMENT=".$method->payment_params->instalments."\;PERIOD=".$method->payment_params->period."\;INITIAL_AMOUNT=".round($order->cart->full_total->prices[0]->price_value_with_tax/$method->payment_params->instalments,(int)$currency->currency_locale['int_frac_digits'])*100;
			if(empty($vars["delay"])){
				$vars["delay"]=1;
			}
		}
		$vars["user_id"]=$user->user_id;
		$vars["customer_ip"]=$user->user_created_ip;
		$safe_mode = ini_get('safe_mode') == 1 || !strcasecmp(ini_get('safe_mode'), 'On');

		if($safe_mode){
			$vars["upload_folder"]=$method->payment_params->param_folder;
			$vars["bin_folder"]=$method->payment_params->binaries_folder;
		}
		else{
		...
		if($element->payment_params->notification==0){
			return true;
		}
		$safe_mode = ini_get('safe_mode') == 1 || !strcasecmp(ini_get('safe_mode'), 'On');
		if($safe_mode){
		...
		$dbOrder = $orderClass->get((int)@$arrayCaddie['caddie']);
		$order = new stdClass();
		...
		if($vars['response_code']=='17'||$vars['response_code']=='75'){
			...
			$order->history->history_data .= JText::sprintf( 'ORDER_CANCEL_BY_USER');
			$orderClass->save($order);
			$app = JFactory::getApplication();
			$app->redirect(hikashop_completeLink('order&task=cancel_order&order_id='.$order->order_id,false,true));
			return true;
		}
		...
		if(!in_array($vars['status'],array(00))){
			...
			$order->order_status = $element->payment_params->invalid_status;
			$body = JText::sprintf('PAYMENT_NOTIFICATION_ERROR',$vars['message'], $vars['status']).'. '.JText::sprintf( 'ORDER_HAVE_BEEN', $order->order_status )."\r\n\r\n".$order_text;
			$mailer->setSubject(JText::sprintf('PAYMENT_NOTIFICATION_PROBLEM','Atos',$vars['message']));
			$mailer->setBody($body);
			$mailer->Send();
			if($element->payment_params->debug){
				echo 'payment with code '.@$vars['status'].(!empty($vars['failed_reason_code'])?' : '.@$vars['failed_reason_code']:'')."\n\n\n";
			}
			$order->history->history_data = ob_get_clean().'  Bank_response_code:'.$vars['status'].'  Message:'.$vars['message'];
			$orderClass->save($order);
			return false;
		}
		...
		$order->history->history_amount=@$vars['amount'].@$this->accepted_currency[$currency_code];
		$order->history->history_payment_id = $element->payment_id;
		$order->history->history_payment_method =$element->payment_type;
		$order->history->history_data = ob_get_clean().'Id de la transaction: '.@$vars['transaction_id'];
		...
		$mailer->setSubject(JText::sprintf('PAYMENT_NOTIFICATION_FOR_ORDER','Atos',$vars['payment_status'],$dbOrder->order_number));
		$body = JText::sprintf('PAYMENT_NOTIFICATION_STATUS','Atos',$vars['payment_status']).' '.JText::sprintf('ORDER_STATUS_CHANGED',$statuses[$order->order_status])."\r\n\r\n".$order_text;
		$mailer->setBody($body);
		$mailer->Send();
		$orderClass->save($order);
		return true;
		...
		function onPaymentConfiguration(&$element){
			...
			if($safe_mode){
				...
				{
				$this->atos = JRequest::getCmd('name','atos');
				if(empty($element)){
					$element = new stdClass();
						$element->payment_name='SIPS ATOS';
						$element->payment_description='You can pay by credit card using this payment method';
						$element->payment_images='MasterCard,VISA,Credit_card,American_Express';
						$element->payment_type=$this->atos;
			...
			$this->toolbar = array(
				...
				'cancel',
				'|',
				array('name' => 'pophelp', 'target' =>'payment-atos-form')
			);

				hikashop_setTitle('SIPS ATOS','plugin','plugins&plugin_type=payment&task=edit&name='.$this->atos);
				$app = JFactory::getApplication();
				$app->setUserState( HIKASHOP_COMPONENT.'.payment_plugin_type', $this->atos);
