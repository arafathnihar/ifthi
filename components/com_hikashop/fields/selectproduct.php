	...
	protected function getInput() {
		...
		return '
				<span id="product_id_'.$i.'" >
					...
					<input type="hidden" name="'.$this->name.'" value="'.@$this->element->product_id.'" />
				</span>
				'. $popup->display(
							'<img src="'. HIKASHOP_IMAGES.'edit.png" alt="'.JText::_('PRODUCT').'"/>',
							'PRODUCT',
							 hikashop_completeLink("product&task=selectrelated&select_type=menu_".$i."&control=".$this->name,true ),
							'product_link',
							760, 480, '', '', 'link')
						.'
				<a href="#" onclick="document.getElementById(\'product_id_'.$i.'\').innerHTML=\'<input type=\\\'hidden\\\' name=\\\''.$this->name.'\\\' value=\\\'0\\\' />\';return false;" >
