		...
		if(!$done){
			...
			if(empty($usable_rates) && empty($shippingClass->errors)){
				...
				if(is_array($usable_rates) && count($usable_rates)){
					...
					if($app->getUserState( HIKASHOP_COMPONENT.'.shipping_id')=='' && $auto_select_default){
	...
	function shipping(){
		...
		if(empty($usable_rates)){
			...
			if($auto_select_default && empty($shipping_id) && count($usable_rates)){
				...
				$shipping_method = $rate->shipping_type;
			...
			$currentShipping = hikashop_import('hikashopshipping',$shipping_method);
